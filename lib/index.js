...
function routerMachine({ config, options = {}, initialContext = {}, history = history_1.createBrowserHistory(), }) {
...
function useRouterMachine({ config, options = {}, initialContext = {}, history = history_1.createBrowserHistory(), }) {
    const machine = createRouterMachine({ config, options, initialContext, history });
    const [state, send, service] = react_2.useMachine(machine);
    react_1.useEffect(() => {
        handleTransitionEvents(service, history, getRoutes(config));
    }, []);
